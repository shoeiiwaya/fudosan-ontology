...
表記ゆれ・略語・住所/面積/築年の不統一を機械が名寄せ。**382 語の辞書を内蔵**し、
...
### 📖 辞書（`ontology.json`）
- **382 語 / 10 ドメイン** — 売買・賃貸・物件表記・登記・調査・契約・収益・旅館・金融・現場語
...
- 配布: **npx**（Node）も **uvx**（Python）も全ツール（`resolve_term` / `normalize` / `assess_risk`）。出力は Python↔JS でパリティテスト一致。
